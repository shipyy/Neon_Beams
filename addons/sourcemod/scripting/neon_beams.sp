...
public void OnPluginStart()
{
	...
	switch( g_Engine )
	{
		...
		{
			HookEventEx("round_start",				Event_RoundStart,	EventHookMode_PostNoCopy);
			HookEventEx("arena_win_panel",			Event_RoundEnded,	EventHookMode_PostNoCopy);
			HookEventEx("round_end",				Event_RoundEnded,	EventHookMode_PostNoCopy);
			HookEventEx("teamplay_restart_round",	Event_RoundEnded,	EventHookMode_PostNoCopy);
			HookEventEx("teamplay_win_panel",		Event_RoundEnded,	EventHookMode_PostNoCopy);
		}
		default:
		{
			HookEventEx("round_start",				Event_RoundStart,	EventHookMode_PostNoCopy);
...
public Action TimerMake(Handle timer, any client)
{
	...
	if( client == 0 && g_iLateLoad != 0 ) return Plugin_Continue; // Allow clients to lateload. Don't allow plugin to lateload (g_iLateLoad)
...
bool IsBlocked(int client)
{
	...
	if( g_iBlocked == 3 )
	{
		...
		if( client )
			...
			PrintToServer("[Neon Beams] Spawning blocked. There is a problem with the maps config, please see error log.");
		return true;
	} else if( g_iBlocked ) {
		g_iSaveOrTemp[client] = 0;
		if( client )
			...
			PrintToServer("[Neon Beams] Spawning blocked, too many %s.", g_iBlocked == 1 ? "Beams" : "Particles");
		return true;
	}
